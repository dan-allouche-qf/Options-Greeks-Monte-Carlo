...
$$\text{Prix} = 10.4506, \quad \Delta = 0.6368, \quad \Gamma = 0.0188, \quad \mathcal{V} = 37.52$$
...
\caption{Comparaison des trois méthodes pour le calcul du Delta. Chaque barre représente l'estimation Monte Carlo avec son intervalle de confiance à 95\%. La ligne horizontale rouge indique la valeur théorique de Black-Scholes ($\Delta = 0.6368$).}
\label{fig:delta}
\end{figure}
...
\toprule
\textbf{Méthode} & \textbf{Estimation} & \textbf{Écart-type} & \textbf{IC 95\%} & \textbf{Erreur rel.} \\
\midrule
Black-Scholes (ref.) & 0.6368 & -- & -- & -- \\
Différences Finies & 0.6372 & 0.0021 & $\pm 0.0041$ & 0.06\% \\
Pathwise & 0.6370 & 0.0021 & $\pm 0.0041$ & 0.03\% \\
Malliavin & 0.6365 & 0.0152 & $\pm 0.0298$ & 0.05\% \\
\bottomrule
\end{tabular}
\caption{Statistiques détaillées pour Delta ($N = 50\,000$)}
\end{table}

\textbf{Observations clés :}
\begin{itemize}
    \item Les trois méthodes convergent vers la valeur théorique avec une erreur relative $< 0.1\%$
    \item \textbf{Pathwise} et \textbf{Différences Finies} ont des variances comparables
    \item \textbf{Malliavin} présente une variance environ \textbf{7 fois plus grande} (écart-type $\approx$ 7× celui de Pathwise)
...
\begin{tabular}{lcccc}
\toprule
\textbf{Méthode} & \textbf{Estimation} & \textbf{Écart-type} & \textbf{Erreur rel.} \\
\midrule
Black-Scholes (ref.) & 0.01876 & -- & -- \\
Différences Finies & 0.01882 & 0.00052 & 0.32\% \\
Pathwise & \textit{N/A} & -- & -- \\
Malliavin & 0.01871 & 0.00178 & 0.27\% \\
\bottomrule
\end{tabular}
\caption{Statistiques détaillées pour Gamma ($N = 50\,000$)}
\end{table}
...
\textbf{Méthode} & \textbf{Estimation} & \textbf{Écart-type} & \textbf{Erreur rel.} \\
\midrule
Black-Scholes (ref.) & 37.524 & -- & -- \\
Différences Finies & 37.52 & 0.12 & < 0.1\% \\
Pathwise & 37.53 & 0.11 & < 0.1\% \\
Malliavin & 37.51 & 0.85 & < 0.1\% \\
\bottomrule
\end{tabular}
\caption{Statistiques détaillées pour Vega ($N = 50\,000$)}
\end{table}
...
\midrule
Différences Finies & \checkmark & \checkmark & \checkmark & $\sim 1\times$ \\
Pathwise & \checkmark & $\times$ & \checkmark & $\sim 0.85\times$ (meilleur) \\
Malliavin & \checkmark & \checkmark & \checkmark & $\sim 46\times$ (pire) \\
\bottomrule
\end{tabular}
\caption{Applicabilité et efficacité relative des méthodes}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Erreur de discrétisation vs coût de calcul}
\end{table}

\textbf{Recommandation :} Pour la plupart des applications, $n = 100$ pas offre un bon compromis précision/coût. L'erreur de discrétisation devient alors négligeable par rapport à l'erreur Monte Carlo.
...
\midrule
Prix & 10.4491 & 10.4506 & 0.0015 \\
Delta & 0.6175 & 0.6368 & 0.0193 \\
Gamma & 0.0193 & 0.0188 & 0.0005 \\
Vega & 37.47 & 37.52 & 0.06 \\
\bottomrule
\end{tabular}
\caption{Validation EDP vs Black-Scholes analytique}
\end{table}
